...
if (isset($_POST['camp'] ) ) {
    ...
    function liste_passage($camp,$periode)
    {
        include '../connexion/connectpg.php';
        // include '../connexion/function.php';
        $rsan = $bdd->prepare("SELECT idpassage FROM tb_comptage_cacao,tb_passage WHERE tb_passage.id=tb_comptage_cacao.idpassage and tb_passage.type_periode= :tp AND  an_campagne =:an  GROUP BY idpassage,type_periode ", array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
    ...
    function somme_cpte_del($nomtab, $codedp, $idp, $an)
    {
        ...
        $rsc = $bdd->prepare("select sum(fruit_a) as x,sum(fruit_b) as y,sum(fruit_c) as r, sum(fruit_a+fruit_b+fruit_c) as t, sum(fruit_d) as d FROM $nomtab where tb_comptage_cacao.code_dep = '$codedp' and supp=0 and idpassage=$idp and an_campagne ='$an'");
        $rsc->execute();
    ...
    function somme_cpte_poid($nomtab, $codedp, $idp, $an)
    {
        ...
        $rsp = $bdd->prepare("select sum(pese_f) as p FROM $nomtab where tb_comptage_cacao.parcelle_code like '$codeapp%' and supp=0 and idpassage=$idp and an_campagne ='$an'");
        $rsp->execute();
    ...
    while ($rowdel = $sqldel->fetch()) {
        ...
        <?php while ($rowdp = $sqldep1->fetch()) {
            ...
            for ($zt = 1; $zt <= $nbre_decomptage; $zt++) {
                ...
                if ($zt == 1) {
                    $list1 = somme_cpte_del('tb_comptage_cacao', $codedep, $tabliste[0]['idpassage'], $new_an_1);
                }
                if ($zt == 2) {
                    $list2 = somme_cpte_del('tb_comptage_cacao', $codedep, $tabliste[1]['idpassage'], $new_an_1);
                }
                if ($zt == 3) {
                    $list3 = somme_cpte_del('tb_comptage_cacao', $codedep, $tabliste[2]['idpassage'], $new_an_1);
                    $listp3 = somme_cpte_poid('tb_comptage_cacao', $codedep, $tabliste[2]['idpassage'], $new_an_1);
                }
                if ($zt == 4) {
                    $list4 = somme_cpte_del('tb_comptage_cacao', $codedep, $tabliste[3]['idpassage'], $new_an_1);
                    $listp4 = somme_cpte_poid('tb_comptage_cacao', $codedep, $tabliste[3]['idpassage'], $new_an_1);
                }
                if ($zt == 5) {
                    $list5 = somme_cpte_del('tb_comptage_cacao', $codedep, $tabliste[4]['idpassage'], $new_an_1);
                    $listp5 = somme_cpte_poid('tb_comptage_cacao', $codedep, $tabliste[4]['idpassage'], $new_an_1);
                }
                if ($zt == 6) {
                    $list6 = somme_cpte_del('tb_comptage_cacao', $codedep, $tabliste[5]['idpassage'], $new_an_1);
                    $listp6 = somme_cpte_poid('tb_comptage_cacao', $codedep, $tabliste[5]['idpassage'], $new_an_1);
                }
