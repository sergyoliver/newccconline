...
if (isset($_POST['camp'])  ) {
   $periode= $_POST['p'];
    ...
    function somme_cpte_del($nomtab, $codedp, $idp, $an)
    {
        ...
        $codeapp = 'K' . $rest;
      // echo "select sum(fruit_a) as x,sum(fruit_b) as y,sum(fruit_c) as r, sum(fruit_a+fruit_b+fruit_c) as t, sum(fruit_d) as d FROM $nomtab where tb_comptage_cacao.code_dep = '$codedp' and supp=0 and idpassage=$idp and an_campagne ='$an'";
    ...
    function somme_cpte_poid($nomtab, $codedp, $idp, $an)
    {
        ...
        $rsp = $bdd->prepare("select sum(pese_f) as p FROM $nomtab where tb_comptage_cacao.code_dep = '$codedp'  and supp=0 and idpassage=$idp and an_campagne ='$an'");
        $rsp->execute();
